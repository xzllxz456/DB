...
--WHERE 절에 PUBLISHER, PRICE 동시 사용시 IX_BOOK2 사용됨
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠' AND PRICE >= 7000;
SELECT * FROM BOOK WHERE PRICE >= 7000 AND PUBLISHER = '굿스포츠'; -- 순서 상관없이 DB가 알아서 해줌

-- 인덱스가 있는데 사용이 안되는 경우
SELECT * FROM BOOK WHERE PRICE > 10000; -- 단독으로 쓰일때
SELECT * FROM BOOK WHERE PUBLISHER LIKE '대한%'; -- 인덱스 사용됨
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%대한%'; -- 인덱스 사용 안됨 -- 앞쪽에 어떤게 올지 모르기 때문
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%미디어'; -- 인덱스 사용 안됨

-- 원본 데이터가 가공하는 경우 : 인덱스 적용 안됨
