...
*************************************/
--(관리자계정 SYSTEM) 유저생성 권한을 가진 SYTTEM에서 작업
...
------------------
--(SYSTEM) CONNECT, RESOUCE 롤(ROLE)에 있는 권한 확인
SELECT *
  FROM DBA_SYS_PRIVS
...
----(MDGUEST) MADANG.BOOK 테이블 데이터 조회(SELECT) 할 수 있음
SELECT * FROM MADANG.BOOK; -- OK
SELECT * FROM MADANG.CUSTOMER; --권한없어 접근불가 : ORA-00942: table or view does not exist
